...
func (self *KiteDBPage) ToBinary() []byte {
	...
	binary.BigEndian.PutUint32(b4, uint32(self.pageId))
	buff.Write(b4)
	// binary.Write(buff, binary.BigEndian, uint32(self.pageType))
	binary.BigEndian.PutUint32(b4, uint32(self.pageType))
	buff.Write(b4)
	// binary.Write(buff, binary.BigEndian, uint32(self.next))
	binary.BigEndian.PutUint32(b4, uint32(self.next))
	buff.Write(b4)
	// binary.Write(buff, binary.BigEndian, uint32(self.checksum))
	binary.BigEndian.PutUint32(b4, uint32(self.checksum))
	buff.Write(b4)
	// binary.Write(buff, binary.BigEndian, uint32(len(self.data)))
	binary.BigEndian.PutUint32(b4, uint32(len(self.data)))
	buff.Write(b4)
